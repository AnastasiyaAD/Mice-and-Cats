...
import at.ac.tuwien.foop.server.game.state.TunnelNode;
...
public class TunnelCamperCat implements IServerCatState {
    ...
    public Optional<Mouse> move(GameState gameState) {
        ...
        if (nearestMouse.isPresent()) {
            ...
            double speedPerTick = ((double) configuration.catSpeed() / 5) / configuration.tickRate();
            var nextPos = computeMoveToMouse(this.tunnelToGuard, position, speedPerTick);
            this.position = nextPos;
        }
        if (nearestMouse.isPresent()) {
    ...
    private double[] computeMoveToMouse(double[] mousePos, double[] catPos, double distance) {
        ...
        if (Double.isNaN(newCatPos[0]) || Double.isNaN(newCatPos[1])) {
            return mousePos;
        }
        return newCatPos;
    }
